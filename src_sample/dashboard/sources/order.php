...
function viewMans()
{
    global $d, $langadmin, $func, $strUrl, $curPage, $items, $paging, $minTotal, $maxTotal, $price_from, $price_to, $allNewOrder, $totalNewOrder, $allConfirmOrder, $totalConfirmOrder, $allDeliveriedOrder, $totalDeliveriedOrder, $allCanceledOrder, $totalCanceledOrder;
...
function editMan()
{
    global $d, $langadmin, $func, $curPage, $item, $order_detail;

    $id = (!empty($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

    if (empty($id)) {
        $func->transfer(khongnhanduocdulieu, "index.php?com=order&act=man&p=" . $curPage, false);
    } else {
        $item = $d->rawQueryOne("select * from #_order where id = ? limit 0,1", array($id));

        if (empty($item)) {
            $func->transfer(dulieukhongcothuc, "index.php?com=order&act=man&p=" . $curPage, false);
        } else {
...
function saveMan()
{
    ...
    if ($id) {
        ...
        if ($d->update('order', $data)) {
            $func->transfer(capnhatdulieuthanhcong, "index.php?com=order&act=man&p=" . $curPage);
        } else {
            $func->transfer(capnhatdulieubiloi, "index.php?com=order&act=man&p=" . $curPage, false);
        }
    } else {
        $func->transfer(dulieurong, "index.php?com=order&act=man&p=" . $curPage, false);
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        ...
        if (!empty($row)) {
            ...
            $func->transfer(xoadulieuthanhcong, "index.php?com=order&act=man&p=" . $curPage);
        } else {
            $func->transfer(xoadulieubiloi, "index.php?com=order&act=man&p=" . $curPage, false);
        }
