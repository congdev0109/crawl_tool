    ...
    if (!empty($_POST['recaptcha_response'])) {
        ...
        try {
            ...
            if (!isset($result->success)) {
                $login_failed = true;
                $error = "Lỗi xác thực reCAPTCHA. Vui lòng thử lại.";
            } else if (!$result->success) {
                $login_failed = true;
                $error = "Xác thực reCAPTCHA không thành công. Vui lòng thử lại.";
            } else if (isset($result->score) && $result->score < 0.1) {
                $login_failed = true;
                $error = "Xác thực reCAPTCHA không thành công. Vui lòng thử lại sau.";
            }
        } catch(Exception $e) {
            $login_failed = true;
            $error = "Lỗi kết nối đến máy chủ reCAPTCHA: " . $e->getMessage();
        }
    } else {
        $login_failed = true; 
        $error = "Vui lòng xác nhận reCAPTCHA.";
    }
}
